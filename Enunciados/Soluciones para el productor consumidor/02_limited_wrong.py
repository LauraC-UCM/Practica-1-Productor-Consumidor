...
def add_data(storage, index, pid, data):
    storage[index.value] = pid*1000 + data
    delay(6)
    index.value = index.value + 1


def get_data(storage, index):
    data = storage[0]
    index.value = index.value - 1
    delay()
...
def consumer(storage, index, empty, non_empty):
    for v in range(N):
        non_empty.acquire()
        print (f"consumer {current_process().name} desalmacenando")
        dato =get_data(storage, index)
        empty.release()
...
def main():
    ...
    prodlst = [ Process(target=producer,
                        name=f'prod_{i}',
                        args=(storage, index, empty, non_empty))
